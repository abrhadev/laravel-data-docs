...
2. **Plan commit** (`docs(spdd): …`): extend the story's family canvas with `/spdd-prompt-update`, or create it with `/spdd-reasons-canvas` if no existing class belongs to it (STORY-001-006 only). Then run `/spdd-prompt-update` on each core canvas whose contract changes. Review this as a diff against canvases that stay.
