...
- Response parameters are not filtered by HTTP method or location; every generated Parameter is serialized. The description sentences are the request ones (the pipeline has no response mode), so a nullable response field reads "A null value is accepted.", and a response field whose rules include `Confirmed` (declared, or from a `#[Rule('confirmed')]` string) asks for a `{input}_confirmation` value and publishes that confirmation companion as a response field too (DTO extraction canvas). Parameters are published under the property names, not Laravel Data's mapped input or output names.
