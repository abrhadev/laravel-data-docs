...
Related canvases: parameter metadata pipeline (process, `toParameter`, nested flags, `confirmationCompanion`); requirement resolution (`RequirementDescriptionStage::NULLABLE_SENTENCE`, and the rule that only `RequiredStage` decides requirement, to which the companion is the sanctioned exception); example generation (the final `example` the companion copies); documentation records (`Parameter`, `ParameterLocation`, `ConfirmationCompanion`); cross-field and acceptance (`ConfirmedProcessor` records the companion and its sentences); attribute processing framework (`ReplacedRules` expands a `#[Rule('confirmed')]` string into the `Confirmed` that records it, and skips a replaced one); Scribe strategies (sole production callers).
...
- A `Confirmed` that a later same-class `#[Rule]` replaces records nothing itself, but the replacing rule can only be `confirmed`, which records the companion, so exactly one companion is published (framework canvas). `CrossFieldComparisonTest` pins `#[Confirmed, Rule('confirmed')]`, and a `#[Confirmed]` array-of-Data source that publishes its items and no companion.
